...
export const Subscriptions = ({
  ...
  const [subscriptionTypeValue, setSubscriptionTypeValue] =
    useState("oneMonth");
  const [subscriptionTypeLabel, setSubscriptionTypeLabel] = useState(
    category.oneMonth.toString()
  );
  ...
  const [subscriptionParams, setSubscriptionParams] = useState(
    "&params=" + category.oneMonth.toString() + "_oneMonth"
  );
  ...
  const changeSubscriptionType = (value: string) => {
    ...
    if (value == "custom") {
      ...
      setSubscriptionParams(
        "&params=" + category[value].toString() + "_" + value
      );
    }
  };
  ...
  const productChanged = (productId: any) => {
    ...
    let plusAmount = parseFloat(
      productInfo.promotion.expiredAt < Date.now()
        ? productInfo.cost
        : (productInfo.cost * (100 - productInfo.promotion.discount)) / 100
    );
    if (!productSelected) {
      plusAmount *= -1;
                        ...
                        <TableRow key={`row_${key}`}>
                          ...
                          <TableCell>
                            {item.promotion.expiredAt < Date.now() ? (
                              ...
                              <div className="row">
                                <span className="old-data">${item.cost}</span>$
                                {(item.cost * (100 - item.promotion.discount)) /
                                  100}
                              </div>
                            )}
                          </TableCell>
                          ...
                          <TableCell style={{ textAlign: "center" }}>
                            <Checkbox
                              ...
                              onChange={() => productChanged(item.id)}
              ...
              <>
                ...
                <ModalBody>
                  <div className="flex justify-between">
                    <span className="text-2xl text-sky-500 font-medium">
                      {product.productName}
                    </span>
                    <p className="text-2xl font-medium">
      ...
      {subscription.length == 0 ? (
        <>
          <div className="pt-10 flex flex-col items-center justify-center space-y-3">
            ...
            <Button onPress={() => openProductModal()}>
        ...
        <>
          <div className="w-full overflow-x-auto ml-auto space-x-2 flex p-1">
            <Button onPress={() => openProductModal()} className="ml-auto mb-2">
                ...
                <TableRow key={`row_${key}`}>
                  ...
                  <TableCell>
                    {!item.product.subscription
                      ? ""
                      : item.stripeCurrentPeriodEnd.toDateString()}
                  </TableCell>
                  <TableCell>
                    <Image
                  ...
                  <TableCell style={{ textAlign: "center" }}>
                    {item.product.subscription && (
                      <Button
                        ...
                        variant="light"
                        size="sm"
                        onPress={() => subscriptionProduct(item.product)}
                      >
                        <Plus />
                      </Button>
                    )}
